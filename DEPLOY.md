# PPT 自动生成工作流 — AI Agent 部署手册

> 本文件供 AI Agent（Claude Code / OpenClaw 等）阅读并执行。人类用户请阅读 [README.md](README.md)。
...
```bash
git clone https://github.com/wangjialiang678/ppt-workflow.git
cd ppt-workflow
```
...
```bash
cat api-keys/api-keys.env.example
```
...
| `MODULE_NOT_FOUND: pptxgenjs` | Node.js 依赖未安装 | `cd PPT制作/workspace && npm install` |
| `browserType.launch: Failed to launch` | Chromium 未下载 | `cd PPT制作/workspace && npx playwright install chromium` |
